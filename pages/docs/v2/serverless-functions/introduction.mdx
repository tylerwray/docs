...
You can use Serverless Functions to handle user authentication, form submission, run database queries, call
slack API's, and more.
...
The above example echoes the `body`, path `query`, and `cookie`s, passed with the request object, as a JSON object using helper methods provided through the [Request and Response](/docs/v2/serverless-functions/supported-languages/#node.js-request-and-response-objects) objects.

Then, to test your API just deploy with [Now CLI](/download) from your terminal:

<Snippet dark text="now" />
<Caption>One-command deployment with <GenericLink href="/download">Now CLI</GenericLink>.</Caption>

The resulting deployment will be ready to go with your Node.js Serverless Function. It will provide you with a URL like the following: <https://node-echo-api.now-examples.now.sh/api/?name=zeit>
...
If this behavior is not desired, you can prefix files or folders with either `_` or `.` to prevent them from being created as Serverless Functions.

Here are some examples of which files will become Serverless Functions:
```js
`api/utils` // ✅ all files inside utils
`api/_utils` // ❌ NO files inside _utils
`api/utils/_time.js` // ❌ `api/utils/_time` endpoint is NOT created
`api/utils/date.js` // ✅ `api/utils/date` endpoint created
```
